...
#include <ros_mesh_rgbd_rendering/MeshRGBDRendering.h>
...
public:
  ...
  bool srvMeshRGBDRenderingCallBack(
      ros_mesh_rgbd_rendering::MeshRGBDRendering::Request &req,
      ros_mesh_rgbd_rendering::MeshRGBDRendering::Response &res);
...
public:
  ...
  bool
  createVisualizerWindow(const std::string &window_name = "MeshRGBDRendering",
                         const int width = 640, const int height = 480,
                         const int left = 50, const int top = 50,
                         const bool visible = true);

  bool bindTriangleMesh(std::vector<Eigen::Vector3f> &points,
...
MeshRGBDRendering::MeshRGBDRendering(
    ...
    const std::string &render_texture_mesh_vertex_shader_file,
    const std::string &render_texture_mesh_fragment_shader_file)
  : node_handle_(node_handle), render_texture_mesh_vertex_shader_file_(
                                 render_texture_mesh_vertex_shader_file),
    render_texture_mesh_fragment_shader_file_(
      render_texture_mesh_fragment_shader_file) {

  mesh_rgbd_rendering_service_ = node_handle_.advertiseService(
        "mesh_rgbd_rendering_service",
        &MeshRGBDRendering::srvMeshRGBDRenderingCallBack, this);
}

MeshRGBDRendering::~MeshRGBDRendering() {}

bool MeshRGBDRendering::createVisualizerWindow(const std::string &window_name,
                                               const int width,
                                               const int height, const int left,
                                               const int top,
                                               const bool visible) {
  ...
  if (!glfwInit()) {
    ROS_INFO(
          "MeshRGBDRendering::createVisualizerWindow::Failed to initialize GLFW");
    glfwTerminate();
    return false;
  }
  ...
  if (!window_) {
    ROS_INFO(
          "MeshRGBDRendering::createVisualizerWindow::Failed to create window");
    glfwTerminate();
    return false;
  }
  ...
  if (glewInit() != GLEW_OK) {
    ROS_INFO(
          "MeshRGBDRendering::createVisualizerWindow::Failed to initialize GLEW");
    glfwTerminate();
    return false;
  }
...
*/
bool MeshRGBDRendering::srvMeshRGBDRenderingCallBack(
    ros_mesh_rgbd_rendering::MeshRGBDRendering::Request &req,
    ros_mesh_rgbd_rendering::MeshRGBDRendering::Response &res) {
  ...
  if (!readTextureMeshfromOBJFile(obj_file, triangle_mesh_)) {
    std::cout
        << " MeshRGBDRendering::srvMeshRGBDRenderingCallBack is not successful"
        << std::endl;
    res.success = false;
    return false;
  };
  ...
  for (int i = 0; i < views; i++) {
    ...
    Eigen::Matrix3d rot;
    rot = Eigen::AngleAxisd(vec_rpy[0], Eigen::Vector3d::UnitX()) *
        Eigen::AngleAxisd(vec_rpy[1], Eigen::Vector3d::UnitY()) *
        Eigen::AngleAxisd(vec_rpy[2], Eigen::Vector3d::UnitZ());
  ...
  if (!createVisualizerWindow("MeshRGBDRendering", image_width_, image_height_,
                              50, 50, true)) {
    std::cout
        << " MeshRGBDRendering::srvMeshRGBDRenderingCallBack is not successful"
        << std::endl;
  ...
  shader_->loadShaders(render_texture_mesh_vertex_shader_file_.c_str(),
...
int main(int argc, char **argv) {
  ...
  std::string shader_path =
      ros::package::getPath("ros_mesh_rgbd_rendering") + "/shader/";
  std::string vert_shader_file = shader_path + "TextureSimpleVertexShader.glsl";
  std::string frag_shader_file =
      shader_path + "TextureSimpleFragmentShader.glsl";

  MeshRGBDRendering::Ptr mesh_rgbd_rendering =
      std::make_shared<MeshRGBDRendering>(node, "MeshRGBDRenderingWindows",
                                          vert_shader_file, frag_shader_file);
  ros::spin();
